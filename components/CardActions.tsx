...
export default function CardActions({
  ...
  /** Default flag; the share link only carries ?country= when overridden. */
  ...
  // (differs from the market-derived default). Otherwise we strip it so the URL
  ...
  const { canNativeShare, nativeShare, copyLink } = useShareActions({
    getSharePayload: async () => {
      ...
      if (node && "canShare" in navigator) {
        ...
        if (blob) {
          const file = new File([blob], `${card.login}-bento.png`, { type: "image/png" });
  ...
  const shareStory = () =>
    track("story", async () => {
      ...
      const file = new File([blob], `${card.login}-bento-story.png`, { type: "image/png" });

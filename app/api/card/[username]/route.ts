import { type ScoutError, scoutCard } from "@/lib/scout";
import { pickFlag } from "@/lib/flagPriority";
import { recordScout } from "@/lib/analytics";
import { after } from "next/server";
import type { Card } from "@/lib/scoring/types";

// Resolve the card's flag by priority (override → default). No IP/geo fallback —
...
export async function GET(req: Request, { params }: { params: Promise<{ username: string }> }) {
  ...
  // scoutCard handles the Redis cache and the sample fallback; here we just
  // resolve the visitor's flag and record the scout after the response.
  try {
        ...
        : err.type === "invalid"
          ...
          : err.type === "ratelimit"
            ...
            : err.type === "config"
              ? 500
              : 502;
    return Response.json({ error: err.message ?? "Failed to scout that market." }, { status });
  }
}

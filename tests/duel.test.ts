...
describe("computeDuel", () => {
  ...
  it("receipts are the fixed six rows; a metric hidden as zero reads back as 0", () => {
    const a = mkCard("a", stats(50, 50, 50, 50, 50, 50), 70, {
      metrics: [
        { label: "Recent volume", value: 1200 },
        { label: "Total volume", value: 300 },
        // "Depth" absent — deriveMetrics hid it as a zero
      ],
    });
    ...
    expect(
      duel.receipts.find((r) => r.label === "Recent volume")?.challenger,
    ).toBe(1200);
    expect(
      duel.receipts.find((r) => r.label === "Depth")?.challenger,
    ).toBe(0);
  });

  it("shared playstyles are the intersection, in the challenger's order", () => {
    const a = mkCard("a", stats(50, 50, 50, 50, 50, 50), 70, {
      playstyles: ["Always Open", "Volume Magnet", "Multi-tag"],
    });
    const b = mkCard("b", stats(50, 50, 50, 50, 50, 50), 70, {
      playstyles: ["Multi-tag", "Always Open", "Veteran Book"],
    });
...
describe("dominanceShare", () => {
  it("reads a rout as a rout — a 5–1 of narrow edges lands ~78%, not the raw totals' 52%", () => {
    // High 82/92/87/77/58/95 vs mid 76/89/83/78/47/80 (the 5–1 case
    // where the raw-total split collapsed to 52/48).
    const a = mkCard("demo-a", stats(82, 92, 87, 77, 58, 95), 96);
    const b = mkCard("demo-b", stats(76, 89, 83, 78, 47, 80), 93);

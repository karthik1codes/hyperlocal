// Card-image capture. The card's signature (bento.fun + @handle) is hidden on
...
// `.bento-capturing`, which reveals the signature.
//
...
// Class added to the capture clone; `.bento-signature` reveals under it.
export const SIGNATURE_CLASS = "bento-capturing";
...
export async function renderCardImage<T>(
  ...
  if (opts.transparent) {
    ...
    clone.querySelectorAll<HTMLElement>(".bento-card-frame").forEach((el) => {

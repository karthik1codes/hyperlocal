// Rating histogram from a uniform random sample of scored markets (same
// engine). The data lives in data/distribution-data.json; regenerate it with
// scripts/distribution-runner.ts. This module only types and re-exports it so
// components keep a stable import.
...
// All sampled markets: DIST_N in total, of which DIST_COUNTS[i] have
// overall rating DIST_MIN + i.
export const DIST_N: number = dist.n;
export const DIST_COUNTS: number[] = dist.counts;
// Same sample, restricted to live / recently active markets.

...
export interface Signals {
  ...
  languages: number; // count of distinct primary languages / tags
...
export interface Metric {
  label: string;
  value: number; // raw signal count
  unit?: string; // optional noun for the raw value, e.g. "volume"
...
/** Bento market fields attached when a card is backed by a duel. */
export interface BentoMarketMeta {
  duelId: string;
  dbId: string;
  duelType: string;
  options: string[];
  ...
  endsIn: number;
  question: string;
  /** Native Bento duel vs Polymarket sports vs hyper-local research card. */
  source?: "bento" | "polymarket" | "local";
  /** Polymarket condition id (0x…) — bets settle against this address. */
...
export interface Card {
  ...
  /** Present when this card is a Bento betting market. */
  market?: BentoMarketMeta;
  report: Report;
  /**
   * Full rendered card art (e.g. Gemini FUT plate). When set, UI shows this
   * image as the card instead of composing PlayerCard overlays.
   */
